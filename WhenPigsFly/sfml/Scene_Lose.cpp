...
void Scene_Lose::onEnd()
{
	m_game->window().close();
}

Scene_Lose::Scene_Lose(GameEngine* gameEngine)
	: Scene(gameEngine)
{
	init();
...
void Scene_Lose::sRender()
{
	...
	bkg.setTexture(m_game->assets().getTexture("loseBkg"));
...
void Scene_Lose::sDoAction(const Action& action)
{
